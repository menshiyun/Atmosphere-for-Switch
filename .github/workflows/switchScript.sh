...
curl -H "$API_AUTH" -o latest.json -sL https://api.github.com/repos/menshiyun/dbi/releases/latest
cat latest.json \
  | jq '.name' \
  | xargs -I {} echo {} >> ../description.txt
cat latest.json \
  | grep -oP '"browser_download_url": "\Khttps://[^"]*DBI\.chs\.nro"' \
  | sed 's/"//g' \
...
curl -H "$API_AUTH" -o latest.json -sL https://api.github.com/repos/zdm65477730/Ultrahand-Overlay/releases/tags/2.0.1
cat latest.json \
  | jq '.tag_name' \
